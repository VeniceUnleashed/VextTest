--[[
    VeniceUnleashed - Modiffied LuaUnit
]]
--[[
        luaunit.lua
...
function M.wrapFunctions()
    ...
    -- so just do nothing !
    -- [ Start of modification for Venice Unleashed ]
    ...
    -- ]]
    print([[Use of WrapFunctions() is no longer needed.
Just prefix your test function names with "test" or "Test" and they
will be picked up and run by LuaUnit.
]])
    -- [ End of modification for Venice Unleashed ]
end
...
function TextOutput:endTest(node)
    if node:isSuccess() then
        if self.verbosity > M.VERBOSITY_DEFAULT then
            -- [ Start of modification for Venice Unleashed ]
            -- io.stdout:write("Ok\n")
            print("    " .. self.result.currentNode.testName .. " ... OK")
            -- [ End of modification for Venice Unleashed ]
        else
            -- [ Start of modification for Venice Unleashed ]
            -- io.stdout:write(".")
        ...
        if self.verbosity > M.VERBOSITY_DEFAULT then
            -- print(node.status)
            print("    " .. self.result.currentNode.testName .. " ... " .. node.status)
            print(node.msg)
            --[[
            ...
            -- io.stdout:flush()
            print(string.sub(node.status, 1, 1))
            -- [ End of modification for Venice Unleashed ]
        end
    end
end
...
function M.LuaUnit.isTestName(s)
    ...
    -- [ Start of modification for Venice Unleashed ]
    -- return string.sub(s, 1, 4):lower() == 'test'
    return string.sub(s, 1, 8):lower() == 'vexttest'
    -- [ End of modification for Venice Unleashed ]
end
...
function M.LuaUnit:startSuite(selectedCount, nonSelectedCount)
    self.result = {
        ...
        startDate = os.date('%Y-%m-%dT%H:%M:%S'),
        -- [ End of modification for Venice Unleashed ]
...
function M.LuaUnit:protectedCall(classInstance, methodInstance, prettyFuncName)
    ...
    local function err_handler(e)
        ...
        return {
            ...
            trace = "No debug.traceback() available in VLua"
        }
    end
    ...
    if prettyFuncName then -- we do have the real method name
        ...
        err.trace = "No debug.traceback() available for " .. prettyFuncName .. " with VLua"
        -- [ End of modification for Venice Unleashed ]
    end
...
function M.LuaUnit:runSuite(...)
    ...
    self:registerSuite()
    self:internalRunSuiteByNames(M.LuaUnit.collectTests())
    self:unregisterSuite()
    return self.result.notSuccessCount
end
